    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        if (currentIndex < questions.length - 1) {
          ...
          navigate('/submit', { state: { sessionId } });
        }
      } else {
      ...
      <div className="glass-card p-4 sm:p-6 border-slate-100">
        ...
        <div className="w-full h-2.5 sm:h-3 bg-slate-100 rounded-full overflow-hidden mb-6 flex">
           <motion.div 
             ...
             className="h-full bg-gradient-to-r from-indigo-600 to-violet-600 shadow-lg shadow-indigo-600/20"
           />
        </div>
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center justify-center rounded-full transition-all duration-300 group ${
                      isSelected ? 'w-12 h-12 sm:w-20 sm:h-20 scale-110 sm:scale-110' : 'w-8 h-8 sm:w-12 sm:h-12 bg-white border-2 border-slate-200 hover:border-indigo-400'
                    }`}
                  >

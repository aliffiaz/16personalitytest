...
export default function QuickAssessment({ user }) {
  ...
  const { type, age, country } = location.state || {};
  ...
  useEffect(() => {
    ...
    fetchInitial();
  }, [type, age, country, navigate, user]);
  ...
  if (error && questions.length === 0) {
    return (
      <div className="h-full flex flex-col items-center justify-center">
        ...
        <button onClick={() => navigate('/dashboard')} className="px-6 py-2 bg-app-primary text-white rounded-full font-bold hover:bg-[#006b82]">Return to Dashboard</button>
      </div>
    );
  }
